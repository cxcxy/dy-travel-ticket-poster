...
[English](README.en.md) | 简体中文 | [在线预览 2 种构图与 12 种风格](https://cxcxy.github.io/dy-travel-ticket-poster/)

把一张照片或一组照片，变成好看的旅行票根海报。现在支持两种排版：原有横向照片票根，以及竖向照片、底部信息联的收藏票根。排版和背景可以独立组合。
...
| 老城竖版收藏票根 | <img src="assets/cases/old-town-original.jpg" width="240" alt="老城街道原图"> | <img src="docs/assets/layouts/portrait-reference-linen.webp" width="240" alt="橄榄绿亚麻背景的老城竖版收藏票根"> |

## 能做什么

- 把单张照片做成一张票根海报
- 把一组照片分别做成一组统一风格的票根
- 在“横向照片 + 右侧信息联”和“竖向照片 + 底部信息联”之间选择
- 保留照片中的人物、动物、建筑、车辆和主要内容
- 自动安排标题、日期、编号和票根信息
- 根据排版应用对应默认背景，也可以指定照片主色或整组统一颜色
- 图片会保持完整比例，不拉伸、不变形
...
| 排版 | 参数 | 适合 |
| --- | --- | --- |
| 原有横版 | `landscape`（默认） | 横向场景、人物合影、建筑全景；照片在左，信息联在右 |
| 竖版收藏票根 | `portrait` | 竖向街景、建筑、人像与细长构图；圆角照片在上，信息和大条码在下 |

两种排版都支持照片主色磨砂、用户指定颜色、整组统一色系，以及下方全部 12 种材质/光线背景。默认值有所不同：`landscape` 默认按照片主色，`portrait` 默认使用参考图里的灰橄榄绿织物背景、暖象牙票纸和近黑文字；只要你明确指定其他配色或背景，就会覆盖这个默认值。

| 横版默认效果 | 竖版默认效果 |
| --- | --- |
| <img src="docs/assets/default-subtle-texture.webp" width="360" alt="横版旅行票根默认效果"> | <img src="docs/assets/layouts/portrait-reference-linen.webp" width="360" alt="竖版收藏票根默认效果"> |

直接这样说即可：

```text
把这张图片做成竖版收藏票根，背景使用第10种风格。
```
...
默认风格会跟随排版变化：横版根据照片主色生成克制柔和的细腻哑光纸纹；竖版使用参考图锁定的橄榄绿自然亚麻、暖象牙票纸与近黑文字。

只有想换一种感觉时，才打开[2 种构图与 12 种风格预览](https://cxcxy.github.io/dy-travel-ticket-poster/)，再告诉我排版、序号或中文名。
...
- 横版不指定背景时，会根据每张照片的主要颜色安排细腻哑光纸纹背景。
- 竖版不指定风格时，默认使用参考图的灰橄榄绿织物背景、暖象牙票纸和近黑文字；背景使用约 `4–5px`、轻微不等距且有纱线明暗体积的自然亚麻织纹，并呈现左上亮、右下暗。
- 一组横版图片默认逐张匹配照片主色；一组竖版图片默认保持同一套参考图橄榄绿、暖象牙与近黑配色身份。
...
```text
使用 $dy-travel-ticket-poster，把这张图做成竖版收藏票根，背景按照片主色。
```
...
- 新增 `portrait` 竖版收藏票根排版：竖向照片、横向撕票线、上下齿口、底部信息与大条码。
- `portrait` 默认整套配色调整为参考图的灰橄榄绿织物、暖象牙票纸和近黑文字；显式照片主色、纯色、统一色系和 12 风格仍可覆盖。
- 在线展示页新增横版/竖版对照、竖版首屏案例与复制调用入口。
- 配色、批量清单、PNG 元数据和严格验证都新增布局锁定，避免横版/竖版裁切串用。

## 2026-08-15 更新

- 新增 12 种可以直接选择的背景风格。
- 默认背景改为根据照片主色生成的细腻哑光纸纹，整体克制柔和。
- 支持单张图片和多张图片批量制作。
- 新增在线风格预览页，可以先看效果再选择。
