...
私有子模块必须确保 Codex Cloud 连接的 GitHub 账号也有读取权限。标准票根由本地 Python、Pillow 和固定粗体字体确定性完成；只有裁切必然损失关键主体、需要扩展无语义环境时才需要图片生成/编辑能力。
...
- Python 3.10+、`requirements.txt` 中声明的 Pillow，以及可用的粗体 TTF/OTF/TTC 字体
